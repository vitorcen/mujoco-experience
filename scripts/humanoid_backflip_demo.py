...
def main():
    parser = argparse.ArgumentParser(description="Humanoid Backflip MoCap Demo")
    parser.add_argument("--headless", action="store_true", help="无 GUI 运行")
    parser.add_argument("--duration", type=float, default=5.0, help="Headless 运行时长")
    ...
    motion_path = os.path.join(dm_dir, "motions", "humanoid3d_backflip.txt")
    ...
    print(f"Playing Backflip motion...")
    ...
    if args.record:
        ...
        frames = []
        duration = player.dt * len(player.data_config) * 2 # Play twice
