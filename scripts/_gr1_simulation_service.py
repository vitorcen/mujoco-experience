...
# Importing this registers the 197 `gr1_unified/...` gymnasium envs used by --client.
# The N1.5 gr00t.eval.simulation only does a bare `import robocasa`, which does NOT
# trigger registration -> gym.make would raise NamespaceNotFound: gr1_unified.
...
# The N1.5 `gr00t.eval.simulation._create_single_env` is the KITCHEN variant: it calls
# `gym.make(env, split=config.split, ...)`. The gr1-tabletop env creator
# (create_env_robosuite) has no `split` arg and raises TypeError. The gr1-tabletop variant
# of this file (youliangtan's tree) omits split. Strip it here so the kitchen tree works
# for gr1 envs too — local to this client process, harmless to anything else.
